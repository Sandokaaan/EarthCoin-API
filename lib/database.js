...
function erase(table, item, callBack) {
  MongoClient.connect(url, function(err, db) {
    if (err) 
      throw err;
    var dbo = db.db(dbName);
    dbo.collection(table).deleteMany(item, function(err, obj) {
      if (err) 
        throw err;
      console.log(obj.result.n + 'document(s) deleted');  // a debug message, can be deleted
      db.close();
      if (callBack)
	callBack();
    });
...
function createIndex(table, item, callBack) {
  MongoClient.connect(url, function(err, db) {
    if (err) 
      throw err;
    var dbo = db.db(dbName);
    dbo.collection(table).createIndex(item, function(err, obj) {
      if (err) 
        throw err;
      console.log('table '+table+' indexed by'+item);  // a debug message, can be deleted
      db.close();
      if (callBack)
	callBack();
    });
...
module.exports = {erase, search, searchSort, searchMore, searchCount, insertArray, createIndex};
